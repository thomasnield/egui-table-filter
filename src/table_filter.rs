    ...
    pub trait TableFilterImpl<T> {
        ...
        fn evaluate_array(&self, items: &Vec<T>, exclude_idx: Option<Id>) -> Vec<bool>;
        fn bind(&mut self,
                    id: Id,
                    response: Response,
                    data: &Vec<T>);
    ...
    pub struct TableFilter<T> {
        column_filters: HashMap<Id,Box<dyn ColumnFilter<T>>>
    }

    impl <T: 'static> TableFilter<T> {
        pub fn new() -> Self {
            Self{
                column_filters: HashMap::default()
            }
        }

        pub fn add_column<V: Eq + Hash + Ord + 'static>(
            mut self,
            id: Id,
            ...
            self.column_filters.insert(id,Box::new(ColumnFilterImpl::new(
                id,
                get_value,
                get_string_value,
                str_search_op
    ...
    impl <T> TableFilterImpl<T> for TableFilter<T> {
        fn check_for_reset(&mut self) {
            if self.column_filters.values().any(|cf| cf.reset_called()) {
                self.column_filters.values_mut().for_each(|cf| cf.reset());
            }
        }

        fn evaluate(&self, item: &T) -> bool {
            self.column_filters.values().all(|cf| cf.evaluate(item))
        }

        fn evaluate_array(&self, items: &Vec<T>, exclude_idx: Option<Id>) -> Vec<bool> {
            let evals: Vec<Vec<bool>> = self.column_filters
                .values()
                .filter(|cf| exclude_idx.is_none() || cf.id() != exclude_idx.unwrap() )
                .map(|cf| cf.get_eval_bool_array(items))
                .collect();
        ...
        fn bind(&mut self,
            ...
            if let Some(cf) = self.column_filters.get(&id) {
                ...
                Popup::menu(&response).id(id)
                    ...
                    .show(|ui| {
                        ui.vertical(|ui| {
                            ...
                            let search_input = TextEdit::singleline(&mut cf.search_field)
    ...
    pub trait ColumnFilter<T> {
        ...
        fn search_field(&mut self) -> &mut bool;
        fn unselect<V>(&mut self, v: &V);
        fn select<V>(&mut self, v: &V);
        fn get_value<V>(&self) -> Box<dyn Fn(&T) -> V>;
